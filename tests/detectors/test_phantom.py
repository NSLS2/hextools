...
from hextools.detectors.phantom import PhantomDetector, PhantomArmLogic, PhantomIO, PhantomPixelDataFormat, PhantomTriggerLogic
from ophyd_async.core import DetectorTrigger, TriggerInfo, init_devices, StaticPathProvider, StaticFilenameProvider
...
def phantom_detector_factory():
    def _factory(write_path: Path):
        return PhantomDetector(
            prefix="TEST:PHANTOM",
            path_provider=StaticPathProvider(StaticFilenameProvider("scan"), write_path),
            name="phantom"
        )
    return _factory
...
@pytest.mark.parametrize(("pixel_format", "expected_dtype"),
                         ((PhantomPixelDataFormat.EIGHT,
                          ADBaseDataType.UINT8),
                          (PhantomPixelDataFormat.EIGHT_R,
                           ADBaseDataType.UINT8), *[(fmt, ADBaseDataType.UINT16) for fmt in PhantomPixelDataFormat if fmt not in (PhantomPixelDataFormat.EIGHT, PhantomPixelDataFormat.EIGHT_R)]))
def test_get_downloaded_dtype(phantom_io: PhantomIO, pixel_format: PhantomPixelDataFormat, expected_dtype: ADBaseDataType):
    assert phantom_io.get_downloaded_dtype(pixel_format) == expected_dtype


@pytest.mark.parametrize(("start", "end", "expected_num"),
                         ((0, 1, 2),
                          (-15, 10, 26),
                          (5, 15, 11),
                          (10, 10, 1),
                          (15, 5, 0)))
def test_get_downloaded_frames(phantom_io: PhantomIO, start: int, end: int, expected_num: int):
...
async def test_trigger_logic_setup_download_req_num_greater_than_zero(phantom_trigger_logic: PhantomTriggerLogic, num_frames: int):
    with pytest.raises(ValueError, match="Number of frames to download must be greater than 0!"):
...
async def test_trigger_logic_setup_download(phantom_trigger_logic: PhantomTriggerLogic, post_trig_frames: int, num: int, expected_start: int, expected_end: int):
    ...
    assert await phantom_trigger_logic.driver.download_start_frame.get_value() == expected_start
    assert await phantom_trigger_logic.driver.download_end_frame.get_value() == expected_end


async def test_arm_logic_arm_timeout_waiting_for_trigger(phantom_arm_logic: PhantomArmLogic, monkeypatch):
    monkeypatch.setattr(hextools.detectors.phantom, "DEFAULT_TIMEOUT", 0.1)  # Set a short timeout for the test
    ...
    # Schedule acquisition stop after a short delay, so it takes effect after acquire.set(True)
    ...
    try:
        with pytest.raises(RuntimeError, match="Acquisition stopped while waiting for event trigger!"):
            await phantom_arm_logic.arm()
    finally:
        stop_task.cancel()

async def test_arm_logic_arm_timeout_saving_to_cine(phantom_arm_logic: PhantomArmLogic, monkeypatch):
    monkeypatch.setattr(hextools.detectors.phantom, "DEFAULT_TIMEOUT", 0.1)  # Set a short timeout for the test

    # Pre-set waiting_for_trigger and trigger_received so the first two loops complete immediately
    set_mock_value(phantom_arm_logic.driver.waiting_for_trigger, 1)
    set_mock_value(phantom_arm_logic.driver.trigger_received, 1)
    # Set post_trig_frames to a value that won't match array_counter, so the second loop times out
    set_mock_value(phantom_arm_logic.driver.post_trig_frames, 10)
    # complete_and_valid stays 0 (default), so the timeout handler will see it as incomplete

    with pytest.raises(TimeoutError, match="Received event trigger, but writing to cine was not completed!"):
        await phantom_arm_logic.arm()

async def test_arm_logic_arm_post_trig_frames_incorrect(phantom_arm_logic: PhantomArmLogic, monkeypatch):
    monkeypatch.setattr(hextools.detectors.phantom, "DEFAULT_TIMEOUT", 0.1)  # Set a short timeout for the test
    ...
    set_mock_value(phantom_arm_logic.driver.array_counter, 5)  # Different from post_trig_frames

    with pytest.raises(ValueError, match="Expected number of post trig frames .* does not match actual number .*"):
        await phantom_arm_logic.arm()
...
async def test_arm_logic_arm_success(phantom_arm_logic: PhantomArmLogic, monkeypatch):
    ...
    set_mock_value(phantom_arm_logic.driver.post_trig_frames, 10)
    set_mock_value(phantom_arm_logic.driver.complete_and_valid, 1)
    set_mock_value(phantom_arm_logic.driver.array_counter, 10)  # Matches post_trig_frames

    await phantom_arm_logic.arm()  # Should complete without exceptions
    assert await phantom_arm_logic.driver.download.get_value() == True


async def test_arm_logic_wait_for_idle_timeout(phantom_arm_logic: PhantomArmLogic, monkeypatch):
    monkeypatch.setattr(hextools.detectors.phantom, "DEFAULT_TIMEOUT", 0.1)  # Set a short timeout for the test
    ...
    with pytest.raises(TimeoutError, match="Timeout waiting for download to complete! Target number of downloaded frames: 11"):
        await phantom_arm_logic.wait_for_idle()

async def test_arm_logic_wait_for_idle_success(phantom_arm_logic: PhantomArmLogic, monkeypatch):
    monkeypatch.setattr(hextools.detectors.phantom, "DEFAULT_TIMEOUT", 0.1)  # Set a short timeout for the test
    set_mock_value(phantom_arm_logic.driver.download_start_frame, -5)
    set_mock_value(phantom_arm_logic.driver.download_end_frame, 5)
    set_mock_value(phantom_arm_logic.driver.download, True)

    # Simulate frames being downloaded by incrementing download_start_frame until it exceeds download_end_frame
...
async def test_detector_describe(phantom_detector, pixel_format: PhantomPixelDataFormat, x_size: int, y_size: int, expected_shape: tuple[int, int, int], expected_dtype: ADBaseDataType):
    ...
    await phantom_detector.prepare(TriggerInfo(
...
async def test_detector_full_stack(RE, phantom_detector_factory, tiled_client, monkeypatch):

    monkeypatch.setenv("OPHYD_ASYNC_PRESERVE_DETECTOR_STATE", "YES")  # Ensure detector config is preserved across stages
    ...
    set_mock_value(phantom.writer.file_path_exists, True)
    set_mock_value(phantom.driver.select_pixel_data_format, PhantomPixelDataFormat.P_TEN)
    ...
    # Use callbacks instead of asyncio.create_task, since RE() blocks the test event loop
    ...
    def _on_download(value, **kwargs):
        ...
        with h5py.File(tmp_path / "scan.h5", "w") as f:
            f.create_dataset("/entry/data/data", data=np.random.randint(0, 65536, size=(11, 3, 4), dtype=np.uint16))
    ...
    # Check that the data we get from the stream is what we expect (the random data we wrote to the file in _on_download)
    run = c.values().last()
